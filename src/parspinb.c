...
#define MAXBOARD 8
#define CHIPHSPACE  580   // mils between chip columns
...
void cpin_to_xy( char *clocstr)
{
  int xbase;
  int ybase;
  int cgroup;  // group of 36 chips, 0 .. 
...
int handleloc( char *inlocstr)
{
  ...
  if (isalpha(inlocstr[1]))
    ...
    {
      if (isdigit ( inlocstr[1] ))
	{
         lowval = inlocstr[1]-'0'+26;
        }
      else
	{
          printf(" loc[1] = %d \n", inlocstr[1]);
